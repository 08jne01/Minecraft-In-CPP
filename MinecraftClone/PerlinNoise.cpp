...
PerlinNoise::PerlinNoise(int xSize, int ySize, int seed, int noise, double scale) : xS(xSize), yS(ySize), s(scale), generator(seed), distribution(0.0, noise)

{
...

	{
...

		{
			...
			coords.push_back(random());
			coords.push_back(random());
			buf.push_back(coords);
		}
		grad.push_back(buf);
	}
...

{
	...
	return (dx * grad[intX][intY][0] + dy * grad[intX][intY][1]);
}
